...
function rsmRender() {
    ...
    const abas = RSM_ABAS.map(a =>
        `<button class="rs-aba${rsm.aba === a.id ? ' on' : ''}" onclick="rsmTrocarAba('${a.id}')">${a.rot}</button>`).join('');
    const atual = RSM_ABAS.find(a => a.id === rsm.aba) || RSM_ABAS[0];
    corpo.innerHTML = `
        <div class="rs-abas">${abas}
            <button class="rs-aba tudo${rsm.aba === 'tudo' ? ' on' : ''}" onclick="rsmTrocarAba('tudo')">📜 Roteiro inteiro</button>
        </div>
        <div class="rs-conteudo">
            ${rsm.aba === 'tudo'
                ? RSM_ABAS.map(a => `<h4 class="rs-sec">${a.rot.replace(/^\d+ · /, '')}</h4>${a.fn(d)}`).join('')
                : atual.fn(d)}
        </div>`;
}
